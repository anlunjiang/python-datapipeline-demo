...
* Database connections and ORMs
  ...
  * Query from databases using the SQLA ORM 
...
Alembic is great for tracking schema migrations within your database. Written by the same team who wrote sqlalchemy, the two play
very well together. As an application evolves the schema of the database will inevitably change. It's essentially git for databases, 
allowing you to easily upgrade and rollback changes. This alleviates alot of the pain in manual tracking of database changes. 
No longer will we be confused about why a column is suddenly missing!  
...
Alembic will create a few files that are integral for it to work:
* Versions folder - this is where your migration scripts will be held
* env.py - configuration setup of alembic, you can customise different behaviour for migrations
  * Name of migration script formats
  * Base declared class to extract metadata
  * connection to feed when migrating etc 
* script.py.mako - template for migration scripts autogenerated

Alembic and autogen migration scripts based on the changes to the tables defined by the SQLAlchemy ORM - automagically
`alembic revision --autogenerate -m "genesis - initialise tables"`


Move up and down the alembic tree with
`alembic upgrade head` - upgrades to current top of tree
`alembic downgrade -1` - downgrades version by 1
...
# Later demo ideas building from this
* Orchestration tools - airflow, dagster
* cheap and cheerful visualisations with streamlit - all the way to production level
